...
from bpdfr_simulation_engine.simulation_engine import run_simulation, SimBPMEnv, add_simulation_event_log_header
from bpdfr_simulation_engine.simulation_stats import SimulationResult
...
active_steps = {
    ...
    Steps.METRICS: False,
    Steps.BAYES_OPT: True
}

print_stats = False


def experiment_operations(model: SimulationModel):
    i = model.value
    ...
    if active_steps[Steps.SPLIT_LOG]:
        print('++++++++++++++ Splitting Event Log - %s:  +++++++++++++++++++++++' % proc_name)
        if synthetic:
            split_synthetic_log(proc_name, i == SimulationModel.Loan_B.value)
        else:
...
def split_synthetic_log(proc_name, even):
    for calendar_type in range(1, 5):
        _print_syntetic_header(calendar_type, even)
...
def _print_syntetic_header(calendar_type: int, even: bool):
    tp = ""
    if calendar_type == 1:
        tp = "%s-24" % ("B" if even else "U")
    elif calendar_type == 2:
        tp = "%s-8" % ("B" if even else "U")
    elif calendar_type == 3:
        tp = "%s-8/4" % ("B" if even else "U")
    elif calendar_type == 4:
        tp = "%s-24/A" % ("B" if even else "U")
    print(tp + " process ------------------------------------------")
    ...
    if is_syntetic[proc_name]:
        ...
        for g_size in granule_sizes:
            for angle in trapezoidal_angles:
                if print_stats:
                    print("Simulating Model With Granule Size: %s and Angle: %s " % (str(g_size), str(angle)))
                run_fuzzy_simulation(proc_name, g_size, angle, s_count)
        ...
        sim_info.append(
            run_simulation(
                ...
                json_path=get_file_path(proc_name=proc_name, file_type=FileType.CRISP_JSON, calendar_type=c_typ,
                                        even=even),
                tot_cases=len(fixed_arrival_times),
                is_fuzzy=False,
                stat_out_path=None,
                log_out_path=None,
                starting_at=starting_datetime,
...
def _compute_log_distance_measures(real_log_path, simulated_log_path):
    ...
    results['MTR'] = _compute_resource_allocation_metrics(real_log_path, simulated_log_path)
